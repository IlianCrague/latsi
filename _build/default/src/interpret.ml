...
let next_line cur (Programme p) =
  let choose (Ligne(Nombre n, _) as l1) (Ligne (Nombre m, _) as l2 ) =
    if m < cur || (cur <= n && n < m) then l1 else l2
  in List.fold_left choose (List.hd p) p

let step (p: programme) ctx  =
    let Ligne(Nombre n, instr) = next_line ctx.line p
